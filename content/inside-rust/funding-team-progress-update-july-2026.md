+++
path = "inside-rust/2026/07/21/funding-team-progress-update-july-2026"
...
We have so far received $350K thanks to generous sponsorship from Google, Amazon, and OpenAI, and the Leadership Council.
...
Of the respondents, **34** indicated interest in a general maintenance (MiR) role and **13** indicated interest in being funded towards specific *project goals* (some people indicated both).

We also asked about desired arrangement, compensation, and pay scale. As expected there was a wide variance. Many people would prefer employment, but some are looking for contracts. Many people are interested in full-time, but many also would prefer half-time or quarter-time. And so forth.

## Hashing out the details of the MiR program

The goal of the MiR program is to provide **open-ended funding to support people in supporting their Rust project teams**. As we discussed in our blog post ["What is Maintenance, anyway?"][wima], the precise needs of teams vary, so this can be tricky to define.

The important thing is that MiRs help "enable other contributors to evolve and improve the project". For some teams, this might mean helping to run triage meetings, or burning through a PR review backlog. For others, it might mean going heads down on a big refactor that everybody on the team knows is necessary, but nobody has the time to do.
...
* *Do we do this off-ramp?* We want to, but it does mean that we have less total MiRs.
...
**Why not employment?** For many people, employment is the ideal setup (though by no means everyone), but we are not in a position to offer it at this time. As the MiR program evolves, we hope to be able to support long-term employment for established Maintainers in Residence, but that is something we'll have to figure out as the program matures. 
...
* **Shovel-ready.** Between equal-priority needs, we start with the one that can begin soonest.
...
**Short-list of teams.** Based on the identified criteria, we are focused first on the following teams: rustup, rustfmt, clippy, rustdoc, and mods.
...
* [ ] *Funding team:* Narrow down to a short list of candidate MiRs and begin individual discussions
* [ ] *Funding team:* Announce MiR selection
* [ ] *MiRs:* Do your thing.
